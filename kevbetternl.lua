...
nick.check_condition = function (ent)
    ...
    local slowwalk_active = ui.find("Aimbot", "Anti Aim", "Misc", "Slow Walk"):get()
    
    if slowwalk_active and not (nick.ref.ragebot.autopeek:get() and not override_active) then
        return "Walking"
...
nick.items = {
    ...
    tp = {
        tp = nick.ref.ragebot.main:switch("Auto TP"),
        inair = nick.ref.ragebot.main:switch("Auto TP In Air"),
        idealtick_override = nick.ref.ragebot.main:switch("Idealtick Override"),
    },
    antiaim = {
...
nick.create_elements = {
    ...
    tp = {
        tp = nick.items.tp.tp:create(),
        inair = nick.items.tp.inair:create(),
        idealtick_override = nick.items.tp.idealtick_override:create(),
    },
    antiaim = {
...
nick.autotp = function ()
    ...
    local tp = nick.items.tp.tp:get()
    local inair = nick.items.tp.inair:get()
...
nick.defensive_aa = function ()
    ...
    local pitch_o = ({
        ...
        ["Random Jitter"] = flick_clock and (math.random(0, 1) == 0 and -89) or 89,
        ["45 deg"] = flick_clock and -45 or 89,
        ["45 deg up and down"] = flick_clock and (math.random(0, 1) == 0 and -45 or 45) or 89,
            ...
            if rage.exploit:get() then
                if nick.aa_ui[i].defensive:get() then
                    ...
                    local yaw_o = ({
                        ...
                        ["Random Side-Way"] = localplayer.choking * 90 + utils.random_float(-30, 30),
